/**
 * Episode 3 render — ffmpeg 9:16 + Typecast VO. Not part of `pnpm build`.
 *
 *   pnpm desk:render-ep3
 *
 * Screenbar only. No arm clips. No on-screen [광고]. Does not upload.
 */
...
const VO = path.resolve(ROOT, 'content/desk/uploads/03-lamp-screenbar/vo.wav');
const STICKER = path.resolve(ROOT, 'public/desk/dev/lamp-screenbar.png');
...
const PLATES = path.join(OUT, 'ep3-plates');
const PROXY = path.join(OUT, 'ep3-lamp-screenbar.proxy.mp4');
const FINAL = path.join(OUT, 'ep3-lamp-screenbar-vo.mp4');
const PACK = path.resolve(ROOT, 'content/desk/uploads/03-lamp-screenbar/video.mp4');
...
const SHOTS: Shot[] = [
  { file: 'lamp-screenbar-20412137.mp4', sourceStart: 1.2, duration: 2.2 },
  { file: 'lamp-screenbar-38133087.mp4', sourceStart: 1.0, duration: 2.8 },
  { file: 'lamp-screenbar-34385703.mp4', sourceStart: 1.0, duration: 3.0 },
  { file: 'lamp-screenbar-12346838.mp4', sourceStart: 0.4, duration: 2.2 },
];

const CAPTIONS = [
  { file: 'cap0.png', text: '모니터가 눈부셔요?', start: 0, end: 2.2 },
  {
    file: 'cap1.png',
    text: '천장 불은 화면에 반사돼요',
    start: 2.2,
  ...
  { file: 'cap2.png', text: '바만 달면 화면만 밝아져요', start: 5.0, end: 8.0 },
...
function verify(): void {
  ...
  if (Number.isNaN(duration) || duration < 9.0 || duration > 11) {
  ...
  writeFileSync(
    path.join(OUT, 'ep3-lamp-screenbar.json'),
    `${JSON.stringify(
      {
        output: FINAL,
        pack: PACK,
        ...
        vo: '예슬 / 일반 / 1.0x',
        notes: [
          'No arm-nb-f80 clip reuse; screenbar stock only',
