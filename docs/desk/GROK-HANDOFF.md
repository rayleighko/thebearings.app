...
지금: **48–72시간 지표만.** 3편 YT 546(21:10) / 쿠팡 클릭 0은 당일 정상. 내일 `gi7YPbNxdY`만 보면 됨. 검색 실적·ep4·블로그 글·사업자·사이트 이중 CTA는 클릭이 오기 전에 시작하지 않음.  
...
대기 중 그록이 **해도 되는 유일한 일:** 운영자가 붙여 준 쿠팡·네이버·유튜브 숫자를 §6에 한 줄 넣고, 암 vs 스크린바 클릭만 한 문장으로 말하기. 렌더·새 SKU·코드 금지.
...
2. 이미 해본 틀린 처방
3. SKU 이름은 말하되 **다는 법/고르는 기준 하나는 댓글에** (열린 고리). “달면 해결돼요”로 닫지 말 것
4. `링크는 댓글에 있어요.`
...
| 소스 | 보는 숫자 | 비고 |
|---|---|---|
| 쿠팡 파트너스 → 일별 실적 | 클릭·구매·수익 | **KPI.** 암/스크린바는 링크가 다름 → 표에서 분리. 같은 링크면 합산됨 |
...
1. **48–72시간 지표** — 쿠팡 일별: 암 `gi6GpRFFBI` vs 스크린바 `gi7YPbNxdY` 클릭(링크가 다르면 표가 갈림). 네이버 커넥트 클릭. 유튜브는 실시간 조회만. 검색 실적·Vercel pageview는 주 1회면 됨. 새 분석 툴(GA4 등) 붙이지 말 것.
