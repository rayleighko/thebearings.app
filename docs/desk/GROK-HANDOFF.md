...
**2026-08-20 — 4편 실행.** SKU `lamp-screenbar`. 훅: 밤 천장 불(잘못된 처방). 암은 샵에만 두고 히어로로 안 판다. 스탠드·키보드·마우스·식품 히어로 금지.
...
지금 그록: `content/desk/uploads/04-lamp-screenbar/script.json` 잠금대로 스톡+렌더. 대본 문구를 바꾸지 말 것. 1–3편 클립·암 클립 재사용 금지. `04-arm-nb-f80` 은 취소.
...
2. **지표** — **메인 클릭 = 네이버 커넥트 카드.** 유튜브는 조회·시청유지(도달). 쿠팡은 desk/`go`가 열린 뒤.
3. **다음 숏폼** — 같은 9:16을 두 곳에. 도달은 유튜브, 결제는 네이버 카드. §11 SKU. 스탠드 단독 금지.
