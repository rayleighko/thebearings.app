...
- 유튜브 프로필 링크 **있음** = `desk.thebearings.app` (apex). `/dev`는 없음 → 한 탭 더. 가능하면 프로필을 `/dev`로 바꾸면 됨. 링크가 없는 게 아님.
...
다음 편 VO: `프로필에 목록 있어요.` **구독하고 링크 방문** 금지 (행동 두 개, 구매와 무관).
...
1. **프로필 (30초)** — 유튜브 링크를 `https://desk.thebearings.app/dev`로 바꾸면 한 탭 줄어듦. apex도 클릭은 됨.
...
| 2 | 유튜브 도달 → 프로필 `desk` → 카드 → 쿠팡/`go` | 프로필 있음. 다음 편 CTA를 프로필로 |
